...
/**
 * The InsertPlanNode identifies a table into which tuples are inserted.  InsertPlanNode标识插入元组的表。
 *
...
itself, i.e. a "raw insert", or will come from the child of the node.
  要插入的值要么被嵌入到InsertPlanNode本身，即“原始插入”，要么来自于节点的子节点
 *
 * NOTE: To simplify the assignment, InsertPlanNode has at most one child. 为了简化赋值，InsertPlanNode最多有一个子节点。
 */
...
class InsertPlanNode : public AbstractPlanNode {
 public:
  ...
  /**
   * Creates a new insert plan node for inserting raw values.  创建用于插入原始值的新插入计划节点。
   * @param raw_values the raw values to be inserted  要插入的原始值
   * @param table_oid the identifier of the table to be inserted into    要插入的表的标识符
   */
  ...
  /**
   * Creates a new insert plan node for inserting values from a child plan.  创建一个新的插入计划节点，用于从子计划插入值。
   * @param child the child plan to obtain values from  子计划从中获取值
   * @param table_oid the identifier of the table that should be inserted into  要插入的表的标识符
   */
  ...
  /** @return The identifier of the table into which tuples are inserted  插入元组的表的标识符*/
   ...
   * ' true '如果我们将插入值直接嵌入到计划中，' false '如果我们有一个提供元组的子计划
  */
 ...
 private:
  /** The raw values embedded in this insert plan (may be empty)  插入计划中嵌入的原始值(可以为空)*/
  std::vector<std::vector<Value>> raw_values_;

  /** The table to be inserted into. 要插入的表的id*/

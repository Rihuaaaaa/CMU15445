 ...
 * AbstractPlanNode表示系统中所有可能的计划节点类型。
  *计划节点被建模为树，所以每个计划节点可以有不同数量的子节点。
  *根据Volcano模型，plan节点接收其子节点的元组。
  *孩子们的顺序可能很重要。
 */

class AbstractPlanNode {      //基类
 public:
  /**
 ...
 private:
   ...
   * 
   * 此计划节点的输出模式。在火山模型中，每个计划节点都会吐出元组，它告诉您该计划节点的元组将具有什么模式。
   */
  const Schema *output_schema_;


  /** The children of this plan node.  该计划节点的子节点 */

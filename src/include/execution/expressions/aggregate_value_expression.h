 ...
 * 
 * 在AggregateExecutor中用到，作用和ColumnValueExpression类似，用于找出属于当前column的这个值。
 * （AggregateExecutor的测试函数在传入outputschme的时候，用于构造的colum不是ColumnValueExpression而是AggregateValueExpression，
 * 就是为了处理不同类型的输入参数。所以才能有以下的用法）说实话有点不太符合逻辑，而且不看测试文件根本不知道有这一用法。
 * 
 * 例：  Value value = column.GetExpr()->EvaluateAggregate(temp.Key().group_bys_, temp.Val().aggregates_);
 */

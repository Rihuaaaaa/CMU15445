...
/*
 ...
 缓冲池管理器支持的可扩展哈希表的实现。支持非唯一键。支持插入和删除。当桶满/空时，表动态增长/收缩。
*/
...
class ExtendibleHashTable {
 public:
  ...
  /**
   * Inserts a key-value pair into the hash table.  向哈希表中插入键值对。
   *
   * @param transaction the current transaction 指向当前事务的指针
  ...
  /**
   * Deletes the associated value for the given key.    删除给定键的关联值。
   *
  ...
  /**
   * Performs a point query on the hash table.    在哈希表上执行点查询。
   *
   * @param transaction the current transaction
 ...
 private:
  ...
  /**
   * KeyToDirectoryIndex - maps a key to a directory index     将键映射到目录索引
   *
   * In Extendible Hashing we map a key to a directory index
  using the following hash + mask function.    在可扩展哈希中，我们使用以下哈希+掩码函数将键映射到目录索引。
   *
  ...
  /**
   * Get the bucket page_id corresponding to a key.   获取与键对应的桶page_id。
   *
  ...
  /**
   * Performs insertion with an optional bucket splitting.  使用可选的桶分割执行插入。
   *
   * @param transaction a pointer to the current transaction    //指向当前事务的指针
   ...
   * Optionally merges an empty bucket into it's pair.  This is called by Remove,
   ...
   可选地将空桶合并到它的pair中。这叫做Remove，如果Remove使桶空。
   *

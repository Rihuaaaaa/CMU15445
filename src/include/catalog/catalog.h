...
/**
 * The TableInfo class maintains metadata about a table.    TableInfo类维护关于表的元数据。所有与表有关的信息被包含在TableInfo中
 * 在bustub中，所有表都被保存在目录Catalog中，可以通过表标识符从中提取对应的TableInfo：
 */


struct TableInfo {       //  TableInfo：装载某个table的相关信息
  ...
  /** An owning pointer to the table heap  指向表堆的所属指针 */
  // 可以通过TableIterator迭代器顺序遍历其中的元组。在SeqScanExecutor中，为其增加TableInfo、及迭代器私有成员，用于访问表信息和遍历表。
  std::unique_ptr<TableHeap> table_;    // 表中的实际元组储存在TableHeap中，其包含用于插入、查找、更改、删除元组的所有函数接口，
...
/**
 * The IndexInfo class maintains metadata about a index.  IndexInfo类维护关于索引的元数据。 
 */
struct IndexInfo {
  /**
 ...
 * The Catalog is a non-persistent catalog that is designed for
 ...
 Catalog是一个非持久性的目录，设计它是为了供DBMS执行引擎中的执行程序使用。它处理表创建、表查找、索引创建和索引查找。
 */
class Catalog {
 public:

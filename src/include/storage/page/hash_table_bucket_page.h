...
class HashTableBucketPage {
 public:
  ...
  /**
   * Remove the KV pair at bucket_idx   移除bucket_idx上的KV对
   */
 ...
 private:
  ...
  char occupied_[(BUCKET_ARRAY_SIZE - 1) / 8 + 1];     // 用于统计桶中的槽是否被使用过,当一个槽被插入键值对时，其对应的位被置为1（访问一次就永久为1）

  // 0 if tombstone/brand new (never occupied), 1 otherwise.
  char readable_[(BUCKET_ARRAY_SIZE - 1) / 8 + 1]; // 数组用于标记桶中的槽是否被占用，当被占用时该值被置为1，否则置为0；

  MappingType array_[0];    //用于存储实际的键值对(key , value)的数组 ，这里要注意 ，桶才是存储实际键值对的，目录只起索引的操作。
};
